...
The post metadata controls are placed directly below the Title and Slug inputs instead of in a right-side column. This keeps Locale, Category, Thumbnail, Published, Tags, and Linked Images close to the primary post fields and gives the markdown editor more room. Thumbnail uses a preview/upload card backed by Supabase Storage and `images` metadata. Tags show only the current post selection as badges; the `New tag` input autocompletes existing tags, creates new tags, and selected badges support rename/delete.
